export default function CommentsTextarea({ label, subLabel, value, placeholder, onChange, readOnly, suggestions }) {
    return (
      <div className="flex flex-col gap-3">
        ...
        <div className="relative">
          <textarea
            ...
            placeholder={placeholder} value={value ?? ""} readOnly={readOnly} onChange={(e) => onChange?.(e.target.value)}
          />
          {!readOnly && (<div className="absolute bottom-3 right-3 text-slate-400 hover:text-primary cursor-pointer" title="Use AI to refine text">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {suggestions.map((s, idx) => (
              <button
                ...
                onClick={() => onChange?.(`${value ? value + ' ' : ''}${s}`)} // optional: append suggestion
              >
                + {s}
              </button>
            ))}
